...
# results is a JSON string webData.close()
print (results)
...
curs.execute('''CREATE TABLE IF NOT EXISTS demo_json (city TEXT, wind_speed NUMERIC)''');
curs.execute('''INSERT INTO demo_json values (?, ?)''', (city, current["speed"]));

conn.commit();
curs.execute('SELECT * FROM demo_json');
